...
Also, please note public cloud images usually do not allow SSH login with user and password but only SSH certificate login.
If you want to use the bootstrap from the {webui}, you will need to enable SSH login with user and password:
...
For information about creating a bootstrap script, see xref:client-configuration:registration-bootstrap.adoc[].
